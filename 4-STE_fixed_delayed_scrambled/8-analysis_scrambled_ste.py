...
def symb_transfer_entropy(source, target, delta, k):
    ...
    calculation is done column (the whole "set" of possible events)
    to row (specific event). Note the delay is only applied to the source
    signal to account for the delays in the system
    Args:
    ...
    int_past = np.concatenate((integer_t[delta-1:-1, :], integer_s[:-delta, :]),
                   axis=1)  # target, source
    int_future = np.concatenate((integer_t[delta:, :], integer_s[delta:, :]),
                   axis=1)  # target, source
...
def main():
    # == 1) Parameters =======================================================
    pair_a = str(sys.argv[1])
    pair_b = str(sys.argv[2])
    freq_band_s = str(sys.argv[3])
    freq_band_t = str(sys.argv[4])
    sub_a = str(sys.argv[5])
    sub_b = str(sys.argv[6])
    ...
    temp = sio.loadmat('2-STE_graham/' + pair_a + '/' + sub_a +
                       '_trials_labels.mat')
    sub_a_trials_labels = [temp['trials_labels'][n][0][0].encode("utf-8")
                           for n in range(21)]
    del temp

    temp = sio.loadmat('/home/horozco/2-STE_graham/' + pair_a + '/' + sub_a +
                       '_source_labels.mat')
    ...
    temp = sio.loadmat('/home/horozco/2-STE_graham/' + pair_b + '/' + sub_b +
                       '_trials_labels.mat')
    ...
    sub_a_baseline_sfreq = sub_a_trials_data[-1] # baseline is always the last one
    ...
    # == 4) Trial Time Frequency Decomposition ===============================
    # We wrangle the data to ensure that the same trial is being analyzed for each person
    sub_a_trials_labels = [sub_a_trials_labels[i].lower() for i in range(len(sub_a_trials_labels))]
    sub_b_trials_labels = [sub_b_trials_labels[i].lower() for i in range(len(sub_b_trials_labels))]
    trial_numbers = ['0'+str(i) for i in range(10)]
    temp_numbers = [str(i) for i in range(10, 21)]
    trial_numbers.extend(temp_numbers)
    sub_a_trials_labels = [sub_a_trials_labels[i] + trial_numbers[i] for i in
                           range(21)]
    sub_b_trials_labels = [sub_b_trials_labels[i] + trial_numbers[i] for i in
                           range(21)]

    trial_labels_a = [sub_a_trials_labels[i] for i in
                      range(len(sub_a_trials_labels)-1)
                      if sub_a_trials_labels[i][-3] != '1']
    trial_labels_b = [sub_b_trials_labels[i] for i in
                      range(len(sub_b_trials_labels) - 1)
                      if sub_b_trials_labels[i][-3] != '1']

    # We have four pieces, so we do this per piece. The order is always the same!
    labels_arranged_a = []
    labels_arranged_b = []

    i_piece = [0, 4, 8, 12]
    for i_piece in i_piece:
        if trial_labels_a[i_piece][-8] != trial_labels_b[i_piece][-8]:
            labels_arranged_a.extend(trial_labels_a[i_piece+2:i_piece+4])
            labels_arranged_a.extend(trial_labels_a[i_piece:i_piece+2])
            labels_arranged_b.extend(trial_labels_b[i_piece:i_piece+4])
        else:
            labels_arranged_a.extend(trial_labels_a[i_piece:i_piece + 4])
            labels_arranged_b.extend(trial_labels_b[i_piece:i_piece + 4])

    indices_labels_arranged_a = [int(labels_arranged_a[i][-2:]) for i in
                                 range(len(labels_arranged_a))]
    indices_labels_arranged_b = [int(labels_arranged_b[i][-2:]) for i in
                                 range(len(labels_arranged_b))]

    for n_trial in range(len(indices_labels_arranged_a)):
        start_time = time.time()
        print('Now processing trial %i' % (n_trial+1))
        # == Source frequency band ===========================================
        # == Sub a ===========================================================
        # Get rid of deep brain structures
        sub_a_trial = sub_a_trials_data[indices_labels_arranged_a[n_trial]]
        ...
        # Downsample HT
        sub_a_trial_s = signal.decimate(sub_a_trial_s_norm, 16, axis=1, zero_phase=True)

        # == Sub b ===========================================================
        # Get rid of deep brain structures
        sub_b_trial = sub_b_trials_data[indices_labels_arranged_b[n_trial]]
        ...
        # Downsample HT
        sub_b_trial_s = signal.decimate(sub_b_trial_s_norm, 16, axis=1,zero_phase=True)
        ...
        if freq_band_s == freq_band_t:
            ...
            # Downsample HT
            sub_a_trial_t = signal.decimate(sub_a_trial_t_norm, 16, axis=1,
                                            zero_phase=True)
            ...
            # Downsample HT
            sub_b_trial_t = signal.decimate(sub_b_trial_t_norm, 16, axis=1,
                                            zero_phase=True)
        ...
        temp_columns = ['sub_source', 'sub_target',
                                         ...
                                         'duo_type', 'piece', 'trial_a',
                                         'trial_b', 'pair_a', 'pair_b', 'ste']
        ...
        temp_df['pair_a'] = pair_a
        temp_df['pair_b'] = pair_b
        temp_df['trial_a'] = labels_arranged_a[n_trial].lower()[-3]
        temp_df['trial_b'] = labels_arranged_b[n_trial].lower()[-3]
        temp_df['duo_type'] = labels_arranged_a[n_trial].lower()[-12]
        temp_df['piece'] = labels_arranged_a[n_trial].lower()[-11:-8]

        if sub_a_trial_s.shape[1] != sub_b_trial_s.shape[1]:
            n_base = min(sub_a_trial_s.shape[1], sub_b_trial_s.shape[1])-1
            sub_a_trial_s = sub_a_trial_s[:, :n_base]
            sub_b_trial_s = sub_b_trial_s[:, :n_base]
            sub_a_trial_t = sub_a_trial_t[:, :n_base]
            sub_b_trial_t = sub_b_trial_t[:, :n_base]

        if labels_arranged_a[n_trial][-8] == 'a':
            ...
            leader = sub_a
            follower = sub_b
        elif labels_arranged_b[n_trial][-8] == 'b':
            ...
            print 'Unable to determine leader and follower, check trial %s' % [labels_arranged_a[n_trial], labels_arranged_b[n_trial]]
            continue
        ...
        if n_trial == 0:
            grand_df = temp_df.copy()
        else:
            grand_df = grand_df.append(temp_df)
        grand_df.to_csv('/home/horozco/8-STE_fixed_delayed_scrambled/fixed_csv_scrambled_matrices/fixed_scrambled_' + pair_a + '_' + pair_b + '_' + freq_band_s + '_' + freq_band_t + '_' + str(delay) + '.csv')
