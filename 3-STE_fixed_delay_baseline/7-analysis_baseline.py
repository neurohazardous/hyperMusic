...
hyperMusic. This was used to analyze baseline files from our participants for
...
3) Time-frequency decomposition of baseline
4) Symbolic Transfer Entropy computation
'''
...
def main():
    ...
    temp_df = pd.read_csv('/home/horozco/3-STE_baseline_graham/baseline_start.csv', header = None)
    event_df = temp_df[temp_df.iloc[:, 0] == pair]
    event_duration_a = event_df[event_df.iloc[:, 1] == sub_a].iloc[0, -1]
    sub_a_baseline_sfreq = sub_a_baseline_sfreq[:, event_duration_a:(event_duration_a + 432000)]  # get 3 minutes of baseline
    ...
    sub_a_base_s = signal.decimate(sub_a_base_s_env_power, 16, axis=1,
                                   zero_phase=True)
    ...
    sub_b_baseline_sfreq = sub_b_baseline_sfreq[:, event_duration_b:(event_duration_b + 432000)]  # get 3 minutes of baseline
    ...
    sub_b_base_s = signal.decimate(sub_b_base_s_env_power, 16, axis=1,
                                   zero_phase=True)
    ...
    if freq_band_s == freq_band_t:
        ...
        sub_a_baseline_tfreq = sub_a_baseline_tfreq[:, event_duration_a:(event_duration_a + 432000)]  # get 3 minutes of baseline
        ...
        sub_a_base_t = signal.decimate(sub_a_base_t_env_power, 16, axis=1,
                                       zero_phase=True)
        ...
        # trim the file around triggers
        sub_b_baseline_tfreq = sub_b_baseline_tfreq[:, event_duration_b:(event_duration_b + 432000)]  # get 3 minutes of baseline
        ...
        sub_b_base_t = signal.decimate(sub_b_base_t_env_power, 16, axis=1,
                                       zero_phase=True)

    # == 5) Symbolic Transfer Entropy ========================================
    temp_columns = ['sub_source', 'sub_target', 'freq_source', 'freq_target',
                    'neuro_source', 'neuro_target', 'trial', 'pair', 'ste']
    temp_data = np.zeros(((sub_a_base_s.shape[0] + sub_b_base_s.shape[0]) *
                          (sub_a_base_t.shape[0] + sub_b_base_t.shape[0]),
                          len(temp_columns)))
    ...
    for n_source in range(len(patch_labels)):
        ...
        temp_df.loc[(48 * n_source):(48 * n_source + 11), 'ste'] = [
            symb_transfer_entropy(sub_a_base_s[n_source, :],
                                  sub_a_base_t[i, :], delay, 3) for i
            in range(sub_a_base_s.shape[0])]
        ...
        temp_df.loc[(48 * n_source + 12):(48 * n_source + 23), 'ste'] = [
            symb_transfer_entropy(sub_a_base_s[n_source, :],
                                  sub_b_base_t[i, :], delay, 3) for i
            in range(sub_a_base_s.shape[0])]
        temp_df.loc[(48 * n_source + 12):(48 * n_source + 23), 'sub_source'] = sub_a
        temp_df.loc[(48 * n_source + 12):(48 * n_source + 23), 'sub_target'] = sub_b
        ...
        temp_df.loc[(48 * n_source + 24):(48 * n_source + 35), 'ste'] = [
            symb_transfer_entropy(sub_b_base_s[n_source, :],
                                  sub_a_base_t[i, :], delay, 3) for i
            in range(sub_a_base_s.shape[0])]
        temp_df.loc[(48 * n_source + 24):(48 * n_source + 35),'sub_source'] = sub_b
        temp_df.loc[(48 * n_source + 24):(48 * n_source + 35),'sub_target'] = sub_a
        ...
        temp_df.loc[(48 * n_source + 36):(48 * n_source + 47), 'ste'] = [
            symb_transfer_entropy(sub_b_base_s[n_source, :],
                                  sub_b_base_t[i, :], delay, 3) for i
            in range(sub_a_base_s.shape[0])]
        temp_df.loc[(48 * n_source + 36):(48 * n_source + 47),'sub_source'] = sub_b
        temp_df.loc[(48 * n_source + 36):(48 * n_source + 47),'sub_target'] = sub_b
   
    temp_df.to_csv('/home/horozco/7-STE_fixed_delay_baseline/baseline_grand_matrices_fixed/' + pair +\
                   '_' + freq_band_s + '_' + freq_band_t + '_baseline_fixed_' +\
                   str(delay) + '.csv')
main()
